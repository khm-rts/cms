...
switch($sort_by)
{
	case 'url':
		$icon_url	= $icon;
		$order_by	= "category_url_key " . strtoupper($order);
		break;
	case 'name':
	default:
		$icon_name	= $icon;
		$order_by	= "category_name " . strtoupper($order);
}
...
if ( isset($_GET['delete'], $_GET['id']) && !empty($_GET['id']) )
{
	...
	if ( $result->num_rows == 1)
	{
		...
		create_event('delete', 'af kategorien ' . $row->category_name, $view_files[$view_file]['required_access_lvl']);
	}
}
?>

<div class="page-title">
	<a class="<?php echo $buttons['create'] ?> pull-right" href="index.php?page=category-create" data-page="category-create"><?php echo $icons['create'] . CREATE_ITEM ?></a>
	<span class="title">
		<?php
	...
	<div class="card-body">
		...
		<div class="table-responsive">
			<table class="table table-hover table-striped">
				...
				$query	=
					"SELECT 
						category_id, category_name, category_url_key
					FROM 
						categories $search_sql";
				$result	= $mysqli->query($query);
				...
				while( $row = $result->fetch_object() )
				{
					...
					<tr>
						...
						<td>/<?php echo $row->category_url_key ?></td>

						<!-- REDIGER LINK -->
						<td class="icon">
							<a class="<?php echo $buttons['edit'] ?>" href="index.php?page=category-edit&id=<?php echo $row->category_id ?>" data-page="category-edit" data-params="id=<?php echo $row->category_id ?>" title="<?php echo EDIT_ITEM ?>"><?php echo $icons['edit'] ?></a>
						</td>

						<!-- SLET LINK -->
						<td class="icon">
							<a class="<?php echo $buttons['delete'] ?>" data-toggle="confirmation" href="index.php?page=<?php echo $view_file; ?>&id=<?php echo $row->category_id ?>&delete" data-page="<?php echo $view_file; ?>" data-params="id=<?php echo $row->category_id ?>&delete" title="<?php echo DELETE_ITEM ?>"><?php echo $icons['delete'] ?></a>
						</td>
					</tr>
					<?php
				}
